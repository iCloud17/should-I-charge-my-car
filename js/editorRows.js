...
// "after <n> <min|hr>" group: a numeric field plus the minutes/hours dropdown,
// shared by the by-duration and station-time tier rows.
function afterField(numExtraCls, numValue, unit) {
  return (
    ...
    `<select class="row-unit" aria-label="Tier start unit">` +
...
export function readTimeFee() {
  ...
  for (const r of document.querySelectorAll("#timeFeeRows .tf-row")) {
    ...
    const unit = r.querySelector(".row-unit").value; // "hr" | "min"
